...
interface MeshWithMeshLineGeometry extends Mesh {
  geometry: MeshLineGeometry & THREE.BufferGeometry;
}
...
function Band({}: BandProps) {
  const band = useRef<MeshWithMeshLineGeometry>(null);
  ...
  useFrame((state, delta) => {
    ...
    // PERBAIKAN: Gunakan MeshLineGeometry setPoints alih-alih TubeGeometry
    if (band.current?.geometry) {
      const points = curve.getPoints(32);
      const meshLineGeometry = band.current.geometry as MeshLineGeometry;
  ...
  // PERBAIKAN UTAMA: Setup texture dengan benar
  useEffect(() => {
    if (texture) {
    ...
    <>
      ...
      <mesh ref={band}>
        ...
        <meshLineMaterial
          color="white"
          depthTest={false}
          resolution={isSmall ? [1000, 2000] : [1000, 1000]} // PERBAIKAN: Konsisten untuk semua device
          useMap
          map={texture}
